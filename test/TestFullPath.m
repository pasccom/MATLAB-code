classdef TestFullPath < matlab.unittest.TestCase
    ...
    methods(Test)
        function testRelative(self, dataOK)
            basePath = fullfile(self.mTestRoot, dataOK{1});
            self.assumeEqual(exist(basePath, 'dir'), 7);

            cd(basePath);
            absPath = fullfile(self.mTestRoot, dataOK{3});
            self.verifyEqual(fullpath(dataOK{2}), absPath);
        end
        function testAbsolute(self, dataOK)
            basePath = fullfile(self.mTestRoot, dataOK{1});
            self.assumeEqual(exist(basePath, 'dir'), 7);
